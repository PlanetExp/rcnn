...
0.2
Added keep_prob placeholder for dropout layers
...
class Model:
	def __init__(self, X, y, keep_prob=0.5):
		self.X = X
		self.y = y
	...
	def inference(self):
		...
		with slim.arg_scope([slim.conv2d],
			...
			net = slim.dropout(reshape, keep_prob=self.keep_prob, scope='dropout')
	...
	with tf.Session() as sess:
		...
		for epoch in range(epochs):
			for batch in range(num_train_batches):
				...
				kp = 0.5
				_, loss, accuracy = sess.run([model.optimize, model.loss, model.evaluate],
					feed_dict={
						X_pl: X,
						y_pl: y,
						keep_prob: kp})
			...
			if epoch % 500 == 0:
				...
				for epoch_test in range(num_test_batches):
					...
					kp = 1.0  # don't dropout any layers when testing
					accuracy_total += sess.run(model.evaluate,
						feed_dict={
							X_pl: X_test,
							y_pl: y_test,
							keep_prob: kp})
